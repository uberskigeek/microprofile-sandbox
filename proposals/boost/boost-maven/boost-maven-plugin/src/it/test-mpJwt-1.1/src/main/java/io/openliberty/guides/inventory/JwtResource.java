...
public class JwtResource {
  ...
  @RolesAllowed({ "admin", "user" })
  @Path("/mpJwtProp")
  public Response getmpJwtProp(@Context SecurityContext securityContext) {
	  
	  String publicKeyLocation = System.getProperty("mp.jwt.verify.publickey.location");
	  if(publicKeyLocation == null) {
		  return Response.ok("public key location property not set").build();
	  }
      return Response.ok(publicKeyLocation).build();
